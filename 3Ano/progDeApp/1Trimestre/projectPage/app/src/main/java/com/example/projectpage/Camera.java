...
public class Camera extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        tiraFoto = v.findViewById(R.id.tiraFoto);
        armazenamento = v.findViewById(R.id.armazenamento);
        if (ActivityCompat.checkSelfPermission(getContext(), android.Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED || ActivityCompat.checkSelfPermission(getContext(), android.Manifest.permission.READ_EXTERNAL_STORAGE)!= PackageManager.PERMISSION_GRANTED){
            ActivityCompat.requestPermissions((Activity) getContext(), new String[]{Manifest.permission.CAMERA}, 0 );
        }
        tiraFoto.setOnClickListener(c ->{
            tirafoto();
        });
        armazenamento.setOnClickListener(c ->{
            pegaFoto();
        });
        return v;
    }
